...
if [ -f "test_mcp_gateway/config.yaml" ] && [ -f "test_mcp_gateway/mcp-gateway-mysql.yaml" ]; then
    echo -e "${GREEN}✅ Found${NC}"
    TESTS_PASSED=$((TESTS_PASSED + 1))
else
...
./mcp-gateway --conf test_mcp_gateway/mcp-gateway-mysql.yaml >/tmp/gateway_test.log 2>&1 &
GATEWAY_PID=$!
...
if [ -n "$MCP_SRV_PID" ] && [ -n "$GATEWAY_PID" ]; then
    ...
    echo -n "Testing MySQL database configuration... "
    config_count=$(mysql -h 127.0.0.1 -P 3311 -u root -pnov24feb11 unla_gateway -e "SELECT COUNT(*) FROM mcp_configs WHERE name='mcp_srv_mgr';" -s -N 2>/dev/null || echo "0")
    if [ "$config_count" -gt 0 ]; then
        echo -e "${GREEN}PASSED (${config_count} configurations)${NC}"
        TESTS_PASSED=$((TESTS_PASSED + 1))
    else
        echo -e "${RED}FAILED${NC}"
        TESTS_FAILED=$((TESTS_FAILED + 1))
